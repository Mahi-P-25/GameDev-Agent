import type { FileIndex, HealthIssue, HealthReport } from '../types';
...
export function analyzeHealth(files: FileIndex): HealthReport {
  ...
  const hasSourceFiles = Object.keys(files).some(
...
export function scanDirectory(paths: string[]): {
  totalFiles: number;
  totalDirs: number;
  configFiles: string[];
