import type { ArchitecturePattern, FileIndex } from '../types';
...
const ARCH_SIGNATURES: ArchSignature[] = [
  {
    ...
    detect: (files: FileIndex) => {
      ...
      const filesWithProvider = Object.entries(files).filter(([, c]) => /\w+\.Provider/.test(c));
      return {
  ...
  },
  {
    ...
    detect: (files: FileIndex) => {
      const providerFiles = Object.entries(files).filter(
        ([, c]) => /Provider/.test(c) && /\w+\.Provider/.test(c),
      );
      return {
  ...
  },
  {
    ...
    detect: (files: FileIndex) => {
      const adapterFiles = Object.keys(files).filter(
        (f) => f.includes('/adapters/') || f.endsWith('Adapter.ts') || f.endsWith('Adapter.tsx'),
      );
      ...
      return {
        ...
        confidence:
          adapterFiles.length > 2
            ? 0.9
            : adapterFiles.length > 0
              ? 0.7
              : hasAdapterInterfaces
                ? 0.6
                : 0,
        evidence: [
  ...
  },
  {
    ...
    detect: (files: FileIndex) => {
      const pageFiles = Object.keys(files).filter(
        (f) => f.includes('/pages/') && (f.endsWith('Page.tsx') || f.endsWith('Page.ts')),
      );
  ...
  },
  {
    ...
    detect: (files: FileIndex) => {
      const serviceFiles = Object.keys(files).filter(
        (f) => f.includes('/services/') && (f.endsWith('.ts') || f.endsWith('.tsx')),
      );
      const hasServiceInterface = Object.values(files).some(
        (c) => /\binterface\s+\w+Service\b/.test(c) || /\binterface\s+\w+ApiClient\b/.test(c),
      );
      return {
        detected: serviceFiles.length > 0 || hasServiceInterface,
        confidence:
          serviceFiles.length > 1 && hasServiceInterface
            ? 0.9
