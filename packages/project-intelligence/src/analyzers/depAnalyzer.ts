import type { DependencyEdge, DependencyGraph, DependencyNode, FileIndex } from '../types';

const IMPORT_RE =
  /(?:import\s+(?:[\w*{},]\s+from\s+)?['"]([^'"]+)['"]|from\s+['"]([^'"]+)['"]|require\(['"]([^'"]+)['"]\))/g;
...
export function analyzeDependencies(files: FileIndex): DependencyGraph {
  ...
  const allFiles = Object.keys(files).filter(
