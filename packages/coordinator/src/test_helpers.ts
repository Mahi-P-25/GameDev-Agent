...
import type { MissionRequest } from './CoordinatorTypes';
...
/**
 * An in-memory EventBus double implementing only the surface the Coordinator
 * uses ({@link publish}/{@link subscribe}). Records every published envelope so
 * assertions can inspect emitted events precisely, keeping tests framework-free
 * and fast — the same double the Project System uses.
 */
...
/** Build a valid {@link MissionRequest} with sensible defaults for tests. */
export function makeRequest(overrides: Partial<MissionRequest> = {}): MissionRequest {
  return {
